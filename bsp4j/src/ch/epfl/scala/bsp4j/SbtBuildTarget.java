 ...
 *
 * <p>For example, say we have a project in `/foo/bar` defining projects `A` and `B` and two meta
 * builds `M1` (defined in `/foo/bar/project`) and `M2` (defined in `/foo/bar/project/project`).
 *
 * <p>The sbt build target for `M1` will have `A` and `B` as the defined targets and `M2` as the
 * parent. Similarly, the sbt build target for `M2` will have `M1` as the defined target and no
 * parent.
 *
 * <p>Clients can use this information to reconstruct the tree of sbt meta builds. The `parent`
