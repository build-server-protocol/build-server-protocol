...
public interface BuildServer {
   ...
   *
   * <p>The client will get a `originId` field in `RunResult` if the `originId` field in the
   * `RunParams` is defined.
   *
   * <p>Note that an empty run request is valid. Run will be executed in the target as specified in
   * the build tool.
   */
