...
public interface BuildClient {
   ...
   *
   * <p>A build/showMessage notification is similar to LSP's window/showMessage, except for a few
   ...
   *
   * <p>A build/logMessage notification is similar to LSP's window/logMessage, except for a few
   * additions like id and originId.
   */
   ...
   * validation runs.
   *
   * <p>Diagnostic is defined as it is in the LSP.
   *
   * <p>When reset is true, the client must clean all previous diagnostics associated with the same
   ...
   * added to the last active diagnostics, allowing build tools to stream diagnostics to the client.
   *
   * <p>It is the server's responsibility to manage the lifetime of the diagnostics by using the
   * appropriate value in the reset field. Clients generate new diagnostics by calling any BSP
   * endpoint that triggers a buildTarget/compile, such as buildTarget/compile, buildTarget/test and
   * buildTarget/run.
   *
   * <p>If the computed set of diagnostic is empty, the server must push an empty array with reset
   * set to true, in order to clear previous diagnostics.
   *
   * <p>The optional originId field in the definition of PublishDiagnosticsParams can be used by
   ...
   *
   * <p>The server may also send additional task notifications for actions not covered by the
   * protocol, such as resolution or packaging. BSP clients can then display this information to
   * their users at their discretion.
   *
   * <p>When beginning a task, the server may send `build/taskStart`, intermediate updates may be
   * sent in `build/taskProgress`.
   *
   * <p>If a `build/taskStart` notification has been sent, the server must send `build/taskFinish`
   * on completion of the same task.
   *
   * <p>`build/taskStart`, `build/taskProgress` and `build/taskFinish` notifications for the same
   * task must use the same `taskId`.
   *
   * <p>Tasks that are spawned by another task should reference the originating task's `taskId` in
