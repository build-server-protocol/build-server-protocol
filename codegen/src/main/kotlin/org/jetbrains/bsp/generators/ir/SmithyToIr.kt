...
class SmithyToIr(val model: Model) {
  ...
  val allDataKindAnnotated: Map<ShapeId, List<PolymorphicData>> = run {
    ...
    val dataKindInhabitants =
        model.getShapesWithTrait(DataKindTrait::class.java).toList().map { shape ->
  ...
  fun definitions(namespace: String): List<Def> =
      model
          .shapes()
          .filter { it.id.namespace == namespace && !it.hasTrait("smithy.api#trait") }
          .flatMap { it.accept(toDefVisitor).stream() }
  ...
  private val toDefVisitor =
      object : ShapeVisitor.Default<List<Def>>() {
        ...
        fun toField(member: MemberShape): Field? {
          ...
          return getType(member.target)?.let { Field(name, it, required, getHints(member)) }
        }

        fun getType(shapeId: ShapeId?): IrShape? =
            shapeId?.let { model.expectShape(it).accept(toIrShapeVisitor) }
        ...
        override fun structureShape(shape: StructureShape): List<Def> {
          ...
          fun makeDiscriminatorField(dataField: Field): Field {
            ...
            return Field(
                "dataKind",
                IrShape(dataKindShapeId(dataField.type.shapeId), Type.String),
                false,
                hints)
          }
          ...
          // this adds a "dataKind" field to the structure, it if contains a "data" (dataWithKind)
          // field
        ...
        override fun intEnumShape(shape: IntEnumShape): List<Def> {
          val enumValues =
              shape.enumValues.map { (name, value) ->
          ...
          return when (shape.expectTrait(EnumKindTrait::class.java).enumKind) {
            EnumKindTrait.EnumKind.OPEN ->
                listOf(
                    Def.OpenEnum(
                        shape.id, EnumType.IntEnum, enumValues.sortedBy { it.value }, hints))
        ...
        override fun enumShape(shape: EnumShape): List<Def> {
          ...
          return when (shape.expectTrait(EnumKindTrait::class.java).enumKind) {
            EnumKindTrait.EnumKind.OPEN ->
                listOf(
                    Def.OpenEnum(
                        shape.id, EnumType.StringEnum, enumValues.sortedBy { it.name }, hints))
            EnumKindTrait.EnumKind.CLOSED ->
        ...
        override fun documentShape(shape: DocumentShape): List<Def> {
          ...
          // A document shape with the data trait in fact defines two structures: one for the data
        ...
        fun typeShape(shape: Shape): List<Def> {
          ...
          return listOfNotNull(irShape?.let { Def.Alias(shape.id, it.type, hints) })
        }
  ...
  val toIrShapeVisitor =
      object : ShapeVisitor.Default<IrShape?>() {
        ...
        override fun mapShape(shape: MapShape): IrShape? {
          return shape.key.accept(this)?.let { key ->
            shape.value.accept(this)?.let { value -> IrShape(shape.id, Type.Map(key, value)) }
          }
        }
        ...
        override fun memberShape(shape: MemberShape): IrShape? =
            model.expectShape(shape.target).accept(this)
      }

  fun getHints(shape: Shape): List<Hint> {
    val documentation =
        shape.getTrait(DocumentationTrait::class.java).map { Hint.Documentation(it.value) }
    val deprecated =
        shape.getTrait(DeprecatedTrait::class.java).map { Hint.Deprecated(it.message.orElse("")) }
