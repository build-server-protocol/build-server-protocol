...
class SmithyToIr(val model: Model, val config: IrConfig) {
  ...
  fun pureMapType(shape: MapShape): Type? =
      shape.key.accept(toTypeVisitor)?.let { key ->
        shape.value.accept(toTypeVisitor)?.let { value -> Type.Map(key, value) }
      }

  val toTypeVisitor =
      object : ShapeVisitor.Default<Type?>() {
        ...
        override fun documentShape(shape: DocumentShape): Type =
            if (shape.hasTrait(DataTrait::class.java) &&
                config.dataWithKind == AbstractionLevel.AsDef)
                Type.Ref(shape.id)
            else Type.Json
        ...
        fun enumUniversal(shape: Shape, openType: Type): Type {
          ...
          return if (config.openEnums == AbstractionLevel.AsType &&
              enumKind == EnumKindTrait.EnumKind.OPEN)
              openType
        ...
        override fun unionShape(shape: UnionShape): Type =
            if (config.untaggedUnions == AbstractionLevel.AsType &&
                shape.hasTrait(UntaggedUnionTrait::class.java)) {
