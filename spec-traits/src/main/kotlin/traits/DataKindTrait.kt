...
class DataKindTrait(builder: Builder) :
    AbstractTrait(ID, builder.sourceLocation), ToSmithyBuilder<DataKindTrait> {
  val kind: String
  ...
  override fun createNode(): Node {
    ...
    val polymorphicDataNodes =
        polymorphicData
            .stream()
            .map { obj: ShapeId -> obj.toString() }
            .map { value: String? -> Node.from(value) }
  ...
  class Provider : TraitService {
    ...
    override fun createTrait(target: ShapeId, value: Node): DataKindTrait {
      ...
      val polymorphicData =
          objectNode.expectMember("extends").expectArrayNode().getElementsAs { node: Node? ->
            ShapeId.fromNode(node)
          }
