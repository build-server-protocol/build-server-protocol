...
1. A global `.bsp/` directory located at a globally accessible location
   (`$XDG_DATA_DIRS/.bsp/`). This location is only reserved for connection
   files generated by build tools installed globally.
...
The `$XDG_DATA_DIRS` is defined in the [XDG Base Directory
Specification](https://specifications.freedesktop.org/basedir-spec/basedir-spec-0.6.html).
For Windows systems, the global location must be `%APPDATA%/.bsp/`.
...
The `argv` command invocation must always create a fresh BSP connection to
a BSP server that listens to client messages in `stdin` and writes responses to
`stdout`. This requirement must be met regardless of the underlying protocol
used by the build tool.

In practice, the `argv` invocation is responsible of setting the environment up
so that BSP clients need no build-tool-specific logic to connect to a BSP
server.
