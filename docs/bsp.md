...
The Build Server Protocol defines a standard to connect to BSP servers.
This standard has been designed such that:

1. Clients don't need to have specific support for a server to be able to
   connect to it.
1. Clients can connect to build tools installed both at the machine or at the
   project level.
1. Different versions of the same build tool can have different connection
   mechanisms.
...
and incorporates changes required to deal with  requirements above.

### The BSP Connection Details

The BSP connection details are modelled in a JSON object generated by a build
tool command. The object contains the following fields to allow clients to
...
trait BspConnectionDetails {
  ...
  /** A sequence of commands to connect to the BSP server that
    * can be executed via system processes. */
...
`<workspace-directory>/.bsp`. The `.bsp/` directory must be at the root of the
workspace base directory and the names of the files must be unique per build
tool and version.

Build servers are free to generate/update the BSP connection details any
time they are executed in a user's workspace. This is an optional feature.

For example, `my-build-tool --bsp-connection-details` writes to
`/code/my-project/.bsp/my-build-tool-213.json` the following JSON object:
...
```json
{
 ...
 "argv": ["/usr/bin/python3", "my-build-tool", "bsp", "--protocol", "tcp", "--host", "localhost", "--port", "3141"]
}
```
...
BSP clients can connect to BSP servers by locating the JSON files in the
standard BSP directory `<workspace-directory>/.bsp` in a project directory.

When more than a BSP connection file is found, the client can use the
connection metadata to select only the BSP servers it's interested in. If there
are still ambiguities, the BSP client is free to choose how to react.
