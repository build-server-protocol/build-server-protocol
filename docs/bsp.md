...
1. [Build Server Protocol](#build-server-protocol)
    ...
    8. [Extensions](#extensions)
        ...
        2. [Sbt](#sbt)
    9. [Connecting to a BSP server](#bsp-server-connection)
...
1. Clients do not need to know the specifics of what kind of connections are
   supported by concrete build tools.
1. Clients can connect to build tools installed both at the machine and at the
   workspace level.
1. Multiple versions of the same build tool can have different connection
   mechanisms.
1. Multiple different BSP servers can run in the same workspace directory.

The following protocol to discover and connect to BSP servers is inspired by
[Jupyter's
Kernelspec](https://jupyter-client.readthedocs.io/en/stable/kernels.html#kernelspecs),
even though it has some notorious differences that are explained below.
...
1. Must be unique per build tool name and version to allow different server
   startup mechanisms per version.
3. Can be updated by the build tool at any point in time, including during the
   startup of the build tool in a workspace.
2. Can be added to version control if and only if they do not contain
...
There are two default locations where build tools can write BSP connection
files:

1. A global `.bsp/` directory located at a globally accessible location
   (`$XDG_DATA_DIR/.bsp/`). This location is only reserved for connection files
   generated by build tools installed globally.
1. A workspace-defined `.bsp/` directory located at the workspace base
   directory (`<workspace-directory>/.bsp/`). This location is used for
   connection files that contain workspace-specific data. It has higher
   priority than the global `.bsp` directory.

Build tools can decide where to write the connection files depending on the
specific implementation details so long as the previous requirements are met.

#### Policy around Connection Files Generation

Build tools are required to implement a command to generate a BSP connection
file in one of the above locations. Such invocation can be run manually by
users or by BSP clients implementing smart discovery capabilities in
a workspace. The invocation must be run in the base directory of that
workspace.

Build tools installed by a user are recommended to generate the BSP connection
files in the global `.bsp/` directory during the installation process, assuming
these files do not need any workspace-specific data to start the BSP server in
the build tool.

#### Build Tool Commands to Start BSP Servers

The most important value of the connection file is the `argv` JSON field. The
value of the `argv` field must be command arguments that are runnable via
system process. The working directory of the execution must be the workspace
base directory and the command invocation must have access to the same
environment variables the BSP client does.

The `argv` command invocation must fire up a BSP server that listens to client
messages in `stdin` and writes responses to `stdout`. This requirement must be
met regardless of the underlying protocol used by the build tool.

In practice, the `argv` invocation is responsible of setting the environment up
so that BSP clients need not build-tool-specific logic to connect to a BSP
server.

In contrast with [Jupyter's connection
files](https://jupyter-client.readthedocs.io/en/stable/kernels.html#connection-files),
this design is:

1. Template-free. There is no need for template variables and other techniques
   to allow the BSP clients pass information to the scripts starting BSP
   servers.
1. Connection-agnostic. We don't need to standardize the different ways clients
   and servers can connect and exchange BSP messages. It's up to the `argv`
   command invocation to run build-tool-specific logic to set up the BSP
   connection.

As the `argv` command invocation is build-tool-specific, build tools can also
implement support for the following features:

1. Remote BSP connections independently of BSP clients.
1. Installation of the build tool in case the build tool is not installed in
   a user's machine.
...
BSP clients can connect to BSP servers by locating connection files in the
standard BSP locations. BSP clients must look up connection files first in the
workspace-defined `.bsp` directory and, only if the lookup of a valid
connection file fails, continue the search in the global `.bsp` directory.
...
are still ambiguities, BSP clients arre free to choose how to react, for
example by asking the end user to select a build server.
...
system output and writing to its system input.
