...
class HappyMockServer(base: File, val logger: Logger, implicit val client: LanguageClient) extends AbstractMockServer {
  ...
  def sources(params: SourcesParams): BspResponse[SourcesResult] = {
    ...
    val item1 = SourceItem(asDirUri(sourceDir1), isDirectory = Some(true), generated = false)
    val items1 = SourcesItem(target1, List(item1))

    val sourceDir2 = target2.uri.toPath.resolve("src-gen/")
    val item2 = SourceItem(asDirUri(sourceDir2), isDirectory = Some(true), generated = true)
    ...
    val item3Dir = SourceItem(asDirUri(sourceDir3), isDirectory = Some(true), generated = false)
    val item31 = SourceItem(Uri(sourceFile1.toUri), isDirectory = Some(false), generated = false)
    val item32 = SourceItem(Uri(sourceFile2.toUri), isDirectory = Some(false), generated = false)
    val item33 = SourceItem(Uri(sourceFile3.toUri), isDirectory = Some(false), generated = true)
