...
Formatting will happen as a step of the generation process, i.e. running `bazel run //:generate` will
ensure that the generated files (both generated libraries, website and any others) are formatted correctly.
...
2. (optionally) `cd website && pnpm install` to install the dependencies
   outside of Bazel's sandbox(needed only for IDE support)
